...
Since we are a distributed team working across many timezones, we have the advantage of being able to work on a feature in one timezone and then pass it along for code review in another timezone. This is a strength as we can have work happening around-the-clock, but it can also be a weakness if the code is not actually ready for merging: the code reviewer would have no choice but to assign the PR back for fixes, resulting in a drawn-out 24-hour feedback loop. So for minor issues, the code reviewer should fix them during the review and merge away to limit the feedback loop. The original developer should be sure to take note of the changes that the code reviewer made so that they can refine their skills and may apply them for the next time.
...
Bonus points are given for starting the commit message with an applicable emoji when the code is pushed over to GitHub:
